...
test('numeric sequences', function () {
  ...
  assert.deepStrictEqual(expand('{1..2}{2..3}'), ['12', '13', '22', '23'])
})

test('numeric sequences with step count', function () {
  assert.deepStrictEqual(expand('{0..8..2}'), ['0', '2', '4', '6', '8'])
  assert.deepStrictEqual(expand('{1..8..2}'), ['1', '3', '5', '7'])
})
...
test('alphabetic sequences', function () {
  ...
  assert.deepStrictEqual(expand('{a..b}{b..c}'), ['ab', 'ac', 'bb', 'bc'])
})
...
test('alphabetic sequences with step count', function () {
  ...
  assert.deepStrictEqual(expand('{b..k..2}'), ['b', 'd', 'f', 'h', 'j'])
})
