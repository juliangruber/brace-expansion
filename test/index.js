...
t.test('ignores ${', async t => {
  ...
  t.strictSame(expand('${a,b}${c,d}{e,f}'), ['${a,b}${c,d}e', '${a,b}${c,d}f'])
  t.strictSame(expand('{a,b}${c,d}${e,f}'), ['a${c,d}${e,f}', 'b${c,d}${e,f}'])
  t.strictSame(expand('${a,b}${c,d}{1..3}'), ['${a,b}${c,d}1', '${a,b}${c,d}2', '${a,b}${c,d}3'])
  t.strictSame(expand('x${a,b}x${c,d}x'), ['x${a,b}x${c,d}x'])
})

t.test('empty option', async t => {
  t.strictSame(expand('-v{,,,,}'), [
    '-v', '-v', '-v', '-v', '-v'
  ])
})
...
t.test('nested', async t => {
  t.strictSame(expand('{a,b{1..3},c}'), [
    'a', 'b1', 'b2', 'b3', 'c'
  ])
  t.strictSame(expand('{{A..Z},{a..z}}'),
...
t.test('order', async t => {
  t.strictSame(expand('a{d,c,b}e'), [
    'ade', 'ace', 'abe'
  ])
})

t.test('pad', async t => {
  t.strictSame(expand('{9..11}'), [
    '9', '10', '11'
  ])
  t.strictSame(expand('{09..11}'), [
    '09', '10', '11'
  ])
})
...
t.test('numeric sequences', async t => {
  ...
  t.strictSame(expand('{1..2}{2..3}'), [
    '12', '13', '22', '23'
  ])
})

t.test('numeric sequences with step count', async t => {
  t.strictSame(expand('{0..8..2}'), [
    '0', '2', '4', '6', '8'
  ])
  t.strictSame(expand('{1..8..2}'), [
    '1', '3', '5', '7'
  ])
})

t.test('numeric sequence with negative x / y', async t => {
  t.strictSame(expand('{3..-2}'), [
    '3', '2', '1', '0', '-1', '-2'
  ])
})
...
t.test('alphabetic sequences', async t => {
  ...
  t.strictSame(expand('{a..b}{b..c}'), [
    'ab', 'ac', 'bb', 'bc'
  ])
})

t.test('alphabetic sequences with step count', async t => {
  t.strictSame(expand('{a..k..2}'), [
    'a', 'c', 'e', 'g', 'i', 'k'
  ])
  t.strictSame(expand('{b..k..2}'), [
    'b', 'd', 'f', 'h', 'j'
  ])
})
